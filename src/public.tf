...
locals {
  ...
  http_setting_cookie_name        = format("%s-appgw-be-cookie-name", local.project)
...
resource "azurerm_application_gateway" "api_gateway" {
  ...
  frontend_ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.cmsgateway_public_ip.id
  }

  backend_address_pool {
    name         = local.backend_address_pool_name
    fqdns        = [module.portal_backend.default_site_hostname]
    ip_addresses = []
  }

  backend_http_settings {
    name                  = local.http_setting_name
    host_name             = module.portal_backend.default_site_hostname
    cookie_based_affinity = "Enabled"
    affinity_cookie_name  = local.http_setting_cookie_name
    path                  = ""
    port                  = 80
    protocol              = "Http"
    request_timeout       = 60
    probe_name            = "probe-cms"
  }

  probe {
    host                                      = module.portal_backend.default_site_hostname
    minimum_servers                           = 0
    name                                      = "probe-cms"
    path                                      = "/"
